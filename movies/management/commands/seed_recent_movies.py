...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for idx, (name, image, rating, cast, description, trailer_url, poster_url, genres, languages) in enumerate(movies_data, start=1):
            ...
            screen_name = f'PVR Screen {(idx % 4) + 1}'
            show_offsets = [
                timedelta(hours=idx),
                timedelta(hours=idx + 3),
                timedelta(days=1, hours=idx),
                timedelta(days=1, hours=idx + 3),
                timedelta(days=2, hours=idx),
            ]

            for offset in show_offsets:
                show_time = (timezone.now() + offset).replace(minute=0, second=0, microsecond=0)
