...
def theater_list(request, movie_id):
    ...
    upcoming_shows = Theater.objects.filter(movie=movie, show_time__gte=timezone.now()).order_by('name', 'show_time')

    grouped_theaters = []
    theater_lookup = {}
    for show in upcoming_shows:
        show_local = localtime(show.show_time)
        theater_entry = theater_lookup.setdefault(
            show.name,
            {
                'name': show.name,
                'dates': {},
        ...
        theater_entry['dates'].setdefault(show_local.date(), []).append(show)

    for theater_name, payload in theater_lookup.items():
        grouped_theaters.append(
            {
                ...
                'dates': [
                    ...
                    for show_date, shows in sorted(payload['dates'].items())
                ],
            }
        )
